...
world-atlas 中 `id` 为 `-99` 的少数争议地区（科索沃、北塞浦路斯、索马里兰共用）：表中无映射，显示 `properties.name` 英文名，点击走「无 ADM1」降级路径，不报错。

### 5.2 边界数据

- **国界**：`https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.json`（TopoJSON，`objects.countries`，`Number(f.id)` 归一化前导零）。
- **ADM1**：`https://www.geoboundaries.org/api/current/gbOpen/{a3}/ADM1/` → `meta.simplifiedGeometryGeoJSON`。该 URL 是 `github.com/...…/raw/<sha>/<path>` 形式，**必须改写为 `https://media.githubusercontent.com/media/wmgeolab/geoBoundaries/<sha>/<path>`**（LFS 媒体直链，CORS `*`；jsdelivr/raw.githubusercontent 只返回 131B 的 LFS 指针文件）。改写失败时回退原 URL。
...
1. 抽样 ≥10 国走通「点国家→详情页→点地区→文章→逐级回退」，样本须含：中国、美国（跨经线）、俄罗斯（跨经线）、法国（海外领地）、新加坡（110m 下的最小级国家之一）、至少 1 个无 ADM1 数据国、至少 1 个 `-99` 争议地区（如科索沃）。
